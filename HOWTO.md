...
The most up-to date version of this document is available at:
    ...
    https://github.com/bitbandi/electrum-dmd-server/blob/master/HOWTO.md
...
document, we assume that user is called 'diamond'). We also assume the
diamond user has sudo rights, so we use `$ sudo command` when we need to.
...
you also need to run diamondd and keep a copy of the full blockchain,
which is roughly 125 GB (January 2017). Ideally you have a machine with 16 GB of RAM
and an equal amount of swap. If you have ~2 GB of RAM make sure you limit diamondd 
...
only leaving spare cycles for diamondd. Fast single core CPU power helps for the initial 
...
### Step 1. Create a user for running diamondd and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `diamondd` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser diamond --disabled-password
    $ sudo apt-get install git
    $ sudo su - diamond
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/diamond/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download diamondd

We currently recommend diamondd 2.0.5.7 stable.

If you prefer to compile diamondd, here are some pointers for Ubuntu:

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config automake libtool
    $ sudo su - diamond
    $ cd ~/src && git clone https://github.com/DMDcoin/Diamond.git -b master
    $ cd Diamond/src
    $ make USE_UPNP= -f makefile.unix
    $ strip src/diamondd
    $ cp -a src/diamondd ~/bin

### Step 3. Configure and start diamondd

In order to allow Electrum to "talk" to `diamondd`, we need to set up an RPC
username and password for `diamondd`. We will then start `diamondd` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.Diamond
    $ $EDITOR ~/.Diamond/Diamond.conf

Write this in `Diamond.conf`:

    daemon=1
    txindex=1

rpcuser / rpcpassword options are only needed for non-localhost connections.
you can consider setting maxconnections if you want to reduce diamondd bandwidth
(as stated above)

If you have an existing installation of diamondd and have not previously
...
Allow some time to pass, so `diamondd` connects to the network and starts
downloading blocks. You can check its progress by running:

    $ diamond-cli getinfo

Before starting the Electrum server your diamondd should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start diamondd at boot
time, running as the 'diamond' user. Check your system documentation to
...
We will download the latest git snapshot for Electrum to configure and install it:
    ...
    $ cd ~
    $ git clone https://github.com/bitbandi/electrum-dmd-server.git
    $ cd electrum-dmd-server
...
Electrum reads a config file (/etc/electrum-dmd.conf) when starting up. This
file includes the database setup, diamondd RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-dmd.conf
...
The "configure" script will take care of this and ask you to create a user for running electrum-dmd-server.
If you're using the user `diamond` to run electrum and have added it as shown in this document, run
the following code to add the limits to your /etc/security/limits.conf:

     echo "diamond hard nofile 131072" >> /etc/security/limits.conf
     echo "diamond soft nofile 131072" >> /etc/security/limits.conf
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u diamond -i ulimit -n


Two more things for you to consider:

1. To increase privacy of transactions going through your server
   you may want to close diamondd for incoming connections and connect outbound only. Most servers do run
   full nodes with open incoming connections though.

2. Consider restarting diamondd (together with electrum-dmd-server) on a weekly basis to clear out unconfirmed
...
The magic moment has come: you can now start your Electrum server as root (it will su to your unprivileged user):
    ...
    # electrum-dmd-server start

Note: If you want to run the server without installing it on your system, just run 'run_electrum_dmd_server" as the
unprivileged user.
...
If you want to stop Electrum server, use the 'stop' command:
    ...
    # electrum-dmd-server stop


If your system supports it, you may add electrum-dmd-server to the /etc/init.d directory.
...
safely whenever your machine is rebooted.
    ...
    # ln -s `which electrum-dmd-server` /etc/init.d/electrum-dmd-server
    # update-rc.d electrum-dmd-server defaults
...
diamonds to confirm that everything is working properly.

### Step 13. Join us on IRC, subscribe to the server thread

Say hi to the dev crew, other server operators, and fans on
irc.freenode.net #electrum-dmd and we'll try to congratulate you
on supporting the community by running an Electrum-DMD node.
